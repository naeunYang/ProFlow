...
function optionChange(combo){
    ...
    if(selectedValue === "type"){
        ...
        url.searchParams.append('type', 'mattype'); // 쿼리 매개변수 추가
    ...
    const url = new URL('/material/search', window.location.origin);
...
function updateMode(UpdateBtn){
    ...
    nameInput.onkeyup = function(event){
        enterEvent(event, nameInput, codeInput, typeCombo, weightInput, weightCombo, remarkInput);
    };
...
// 키 이벤트
function enterEvent(event, nameInput, codeInput, typeCombo, weightInput, weightCombo, remarkInput) {
    // 엔터키
    if (event.keyCode === 13) {
        // 1. 필수 입력 검사
        var checkCells = [
            {label: "자재명", value: nameInput.value},
...
function updateTable(materials){
    ...
    materials.forEach(material => {
        ...
        checkbox.setAttribute('onchange', `checkChange(this, '${material.code}')`);
        ...
        remarkDiv.textContent = material.remark;
...
function cnt() {
    ...
    var span = document.getElementById('materialCount');
